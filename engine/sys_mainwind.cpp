...
void CGame::PlayStartupVideos( void )
{
        ...
        // Direct video playback - play valve.bik directly without needing StartupVids.txt
        char localPath[MAX_PATH];
        ...
        if ( bEndGame )
        {
                ...
                // Try to play endgame video if exists
                if ( g_pFileSystem->GetLocalPath( "media/endgame.bik", localPath, sizeof(localPath) ) )
                {
                        PlayVideoAndWait( localPath, false );
                }
        }
        else if ( bRecap )
        {
                // Try to play recap video if exists
                if ( g_pFileSystem->GetLocalPath( "media/recap.bik", localPath, sizeof(localPath) ) )
                {
                        PlayVideoAndWait( localPath, false );
                }
        }
        ...
        {
                // Play valve.bik directly on startup
                if ( g_pFileSystem->GetLocalPath( "media/valve.bik", localPath, sizeof(localPath) ) )
                {
