...
static ConVar cl_soundfile( "cl_soundfile", "sound/player/jingle.wav", FCVAR_ARCHIVE, "Jingle sound file." );
           ConVar cl_avatar( "cl_avatar", "", FCVAR_ARCHIVE | FCVAR_USERINFO, "Custom avatar VTF file path (uploaded like sprays via sv_allowupload)." );
...
void CClientState::CheckOwnCustomFiles()
{
        ...
        if ( IsPC() )
        {
                ...
                AddCustomFile( 2, cl_avatar.GetString() ); // Avatar VTF upload (slot 2) - works like sprays with sv_allowupload
        }
}
