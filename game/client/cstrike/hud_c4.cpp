...
void CHudC4::Paint()
{
        ...
        // Use custom HUD color, preserve flash behavior
        ...
        if ( m_pIcon )
        {
                if( m_bFlash )
                {
                        // Flash to white when in bomb zone
                        m_pIcon->DrawSelf( 0, 0, w, h, flashColor );
                }
                else
                {
                        // Normal state uses HUD color
                        m_pIcon->DrawSelf( 0, 0, w, h, hudColor );
                }
        }
}
