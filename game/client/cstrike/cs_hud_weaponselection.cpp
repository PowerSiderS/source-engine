...
{
        // Modulate the input color with the custom HUD color while preserving state differentiation
        Color hudColor = GetHudColor();
        Color blendedColor;
        
        // Blend the HUD color with the input color to preserve state-based tinting
        // Multiply each channel and normalize (input color is the state modifier)
        blendedColor[0] = (hudColor[0] * color[0]) / 255;
        blendedColor[1] = (hudColor[1] * color[1]) / 255;
        blendedColor[2] = (hudColor[2] * color[2]) / 255;
        blendedColor[3] = color[3];
        
        BaseClass::DrawBox( x, y, wide, tall, blendedColor, normalizedAlpha / 255.0f );

        // draw the number
        if (number >= 0)
        {
                Color numberColor = GetHudColor();
                numberColor[3] = (int)(normalizedAlpha);
