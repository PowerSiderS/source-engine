...
IMPLEMENT_CLIENTCLASS_DT( C_CSPlayer, DT_CSPlayer, CCSPlayer )
    ...
    RecvPropBool( RECVINFO( m_bInHostageRescueZone ) ),
        ...
        RecvPropInt( RECVINFO( m_nInspectParity ) ),
...
void C_CSPlayer::OnDataChanged( DataUpdateType_t type )
{
        ...
        if ( type == DATA_UPDATE_CREATED )
        {
                ...
                m_nLastInspectParity = m_nInspectParity;
        }

        // Handle spam inspect - when parity changes, restart the viewmodel animation
        if ( IsLocalPlayer() && m_nInspectParity != m_nLastInspectParity )
        {
                m_nLastInspectParity = m_nInspectParity;

                // Play the inspect animation on the viewmodel
                C_BaseViewModel *pViewModel = GetViewModel();
                if ( pViewModel )
                {
                        int nSequence = pViewModel->SelectWeightedSequence( ACT_VM_IDLE_LOWERED );
