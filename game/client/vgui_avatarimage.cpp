...
// cl_avatar is owned by the engine module. Use a ConVarRef so the client
// module does not require a direct link to the engine's global symbol.
static ConVarRef s_clAvatar( "cl_avatar", true );
...
//-----------------------------------------------------------------------------
// Purpose: Load avatar for any player using the custom-file CRC sent in
// player_info_t. This is the same CRC/file-transfer path used by sprays.
//-----------------------------------------------------------------------------
bool CAvatarImage::SetAvatarFromNetworkedCRC( int iPlayerIndex )
{
        ...
        CRC32_t avatarCRC = playerInfo.customFiles[CUSTOM_FILE_AVATAR];
        if ( avatarCRC == 0 )
        {
...
// Uses the CRC-based custom-file system like sprays (avatar slot).
//-----------------------------------------------------------------------------
void CAvatarImagePanel::SetPlayer( int entindex, EAvatarSize avatarSize )
{
        m_pImage->ClearAvatarSteamID();

        // Try to load avatar from the networked custom-file CRC.
        ...
        // Fallback: For local player, also check the configured VTF directly
        // before the client has completed a connection/user-info update.
        C_BasePlayer *pLocalPlayer = C_BasePlayer::GetLocalPlayer();
        if ( pLocalPlayer && pLocalPlayer->entindex() == entindex )
        {
                player_info_t pi;
                if ( engine->GetPlayerInfo( entindex, &pi ) &&
                     pi.customFiles[CUSTOM_FILE_AVATAR] != 0 )
                {
                        // Try loading from local custom file
                        if ( m_pImage->SetAvatarFromCRC( pi.customFiles[CUSTOM_FILE_AVATAR] ) )
                        {
                                return;
                        }
                }

                if ( s_clAvatar.IsValid() && s_clAvatar.GetString()[0] &&
                     m_pImage->SetAvatarFromVTFFile( s_clAvatar.GetString() ) )
                {
                        return;
                }
        }
