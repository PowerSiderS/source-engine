...
void CWeaponCSBase::ItemPostFrame()
{
        ...
        else if ( !(pPlayer->m_nButtons & (IN_ATTACK|IN_ATTACK2) ) )
        {
                ...
                // Don't play idle animation while inspecting weapon
                if ( !pPlayer->IsLookingAtWeapon() )
                {
                        WeaponIdle( );
                }
                return;
...
//-----------------------------------------------------------------------------
// Purpose: Helper function to calculate head bob (CS:GO style)
//-----------------------------------------------------------------------------
float CalcNewViewModelBobbing( CBasePlayer *player, BobState_t *pBobState, int nVMIndex )
{
        ...
        // Off-hand viewmodel (index 1) uses simplified bobbing
        bool bIsOffHandViewModel = (nVMIndex == 1);
        ...
        CWeaponCSBase *pWeapon = ((bIsOffHandViewModel || !pPlayer) ? NULL : pPlayer->GetActiveCSWeapon());
        if ( pPlayer && pWeapon )
        {
        ...
        float flBobVert = bIsOffHandViewModel ? 0.3 : cl_bobamt_vert.GetFloat();
        ...
        float flBobLat = bIsOffHandViewModel ? 0.5 : cl_bobamt_lat.GetFloat();
        if ( pPlayer && pWeapon )
        {
...
//-----------------------------------------------------------------------------
// Purpose: Helper function to add head bob (CS:GO style)
//-----------------------------------------------------------------------------
