...
#ifdef GAME_DLL
ConVar cs_AssistDamageThreshold( "cs_AssistDamageThreshold", "40.0", FCVAR_NONE, "Minimum damage dealt to get an assist credit in kill feed" );
#endif
...
ConVar cl_autohelp(
        ...
        //-----------------------------------------------------------------------------
        // Purpose: Find the player who assisted with the kill (dealt most damage besides killer)
        //-----------------------------------------------------------------------------
        CCSPlayer* CCSGameRules::CheckAndAwardAssists( CCSPlayer* pCSVictim, CCSPlayer* pKiller )
        {
                ...
                FOR_EACH_LL( victimDamageTakenList, ii )
                {
                        if ( victimDamageTakenList[ii]->GetPlayerRecipientPtr() == pCSVictim )
                        {
                                ...
                                if ( pAttackerPlayer )
                                {
                                        if ( (victimDamageTakenList[ii]->GetDamage() > maxDamage) && 
                                                 (pAttackerPlayer != pKiller) && 
                                                 (pAttackerPlayer != pCSVictim) )
                                        {
        ...
        void CCSGameRules::DeathNotice( CBasePlayer *pVictim, const CTakeDamageInfo &info )
        {
                ...
                CCSPlayer *pAssister = CheckAndAwardAssists( pCSVictim, ToCSPlayer(pKiller) );

                if ( pAssister )
                {
                        assister_ID = pAssister->GetUserID();
                }
