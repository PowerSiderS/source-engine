...
void CCSPlayer::Spawn()
{
    ...
    if ( pClanTag )
    {
        ...
        // Read cl_avatar from client and apply (Android/non-Steam builds)
        // Avatar VTF path for server-side sharing (works like sprays with sv_uploadurl)
        const char *pAvatarPath = engine->GetClientConVarValue( engine->IndexOfEdict( edict() ), "cl_avatar" );
        if ( pAvatarPath )
        {
                SetAvatarPath( pAvatarPath );
        }
...
void CCSPlayer::SetAvatarPath( const char *pPath )
{
        if ( pPath )
        {
                Q_strncpy( m_szAvatarPath, pPath, sizeof( m_szAvatarPath ) );
        }
}
