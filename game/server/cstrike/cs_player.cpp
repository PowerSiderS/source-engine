...
bool CCSPlayer::HandleCommand_JoinTeam( int team )
{
	...
	*/
	...
	// This must come before ClientKill() for CheckWinConditions() to function properly
		...
		bool bPlayingTeam = ( GetTeamNumber() == TEAM_CT || GetTeamNumber() == TEAM_TERRORIST );
		if ( CSGameRules() && CSGameRules()->IsSwitchingTeamsAtRoundReset() && !WillSwitchTeamsAtRoundReset() && bPlayingTeam )
...
void CCSPlayer::Spawn()
{
    ...
    if ( pClanTag )
    {
        ...
        {
                SetAvatarCRC( pi.customFiles[2] ); // Slot 2 is avatar (slot 0 is spray, slot 1 is sound)
        }
...
void CCSPlayer::SetAvatarCRC( CRC32_t crc )
{
        m_nAvatarCRC = crc;
}
void CCSPlayer::CreateRagdollEntity()
...
//=============================================================================
                ...
                {               
                        // Record for the shooter
                        pPlayer->RecordDamageGiven( this, info.GetDamage() );

                        // And for the victim
                        RecordDamageTaken( pPlayer, info.GetDamage() );
                }
                else
                {
                        RecordDamageTaken( NULL, info.GetDamage() );
                }
...
//=======================================================
// CDamageRecord constructor and helper methods
//=======================================================
CDamageRecord::CDamageRecord( CCSPlayer *pPlayerDamager, CCSPlayer *pPlayerRecipient, int iDamage, int iCounter )
{
...
{
        if ( m_PlayerDamager.IsValid() && m_PlayerDamager.Get() == pPlayerDamager &&
                 m_PlayerRecipient.IsValid() && m_PlayerRecipient.Get() == pPlayerRecipient )
        {
                return true;
        }
        return false;
...
CCSPlayer* CDamageRecord::GetPlayerDamagerPtr( void )
{
        if ( m_PlayerDamager.IsValid() )
                return m_PlayerDamager.Get();
        return NULL;
}

CCSPlayer* CDamageRecord::GetPlayerRecipientPtr( void )
{
        if ( m_PlayerRecipient.IsValid() )
                return m_PlayerRecipient.Get();
        return NULL;
}
...
// Remember this amount of damage that we dealt for stats
//=======================================================
void CCSPlayer::RecordDamageGiven( CCSPlayer *pDamageTaker, int iDamageGiven )
{
        FOR_EACH_LL( m_DamageGivenList, i )
        {
                if( m_DamageGivenList[i]->IsDamageRecordStillValidForDamagerAndRecipient( this, pDamageTaker ) )
                {
                        m_DamageGivenList[i]->AddDamage( iDamageGiven, s_BulletGroupCounter );
                        return;
                }
        ...
        CDamageRecord *record = new CDamageRecord( this, pDamageTaker, iDamageGiven, s_BulletGroupCounter );
        int k = m_DamageGivenList.AddToTail();
        m_DamageGivenList[k] = record;
}
...
// Remember this amount of damage that we took for stats
//=======================================================
void CCSPlayer::RecordDamageTaken( CCSPlayer *pDamageDealer, int iDamageTaken )
{
        FOR_EACH_LL( m_DamageTakenList, i )
        {
                if( m_DamageTakenList[i]->IsDamageRecordStillValidForDamagerAndRecipient( pDamageDealer, this ) )
                {
                        m_DamageTakenList[i]->AddDamage( iDamageTaken, s_BulletGroupCounter );
                        return;
                }
        ...
        CDamageRecord *record = new CDamageRecord( pDamageDealer, this, iDamageTaken, s_BulletGroupCounter );
        int k = m_DamageTakenList.AddToTail();
        m_DamageTakenList[k] = record;
}
